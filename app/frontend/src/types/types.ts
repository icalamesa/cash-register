/**
 * This file was auto-generated by openapi-typescript.
 * Do not make direct changes to the file.
 */

export interface paths {
    "/cart/add": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Add a product to the cart
         * @description Add a product to the cart by specifying the product code and quantity.
         */
        post: operations["add_to_cart_cart_add_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/cart/list": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * List all items in the cart with pricing details
         * @description Retrieve all products currently in the cart, along with their quantities, original prices, and discounted prices (if applicable).
         */
        get: operations["list_cart_cart_list_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/cart/clear": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Clear the cart
         * @description Remove all products from the cart.
         */
        post: operations["clear_cart_cart_clear_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/catalog/list": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * List all items in the catalog with its name and original price
         * @description Retrieve all products existing in the catlog, along with their prices (without discounts)
         */
        get: operations["list_catalog_catalog_list_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
}
export type webhooks = Record<string, never>;
export interface components {
    schemas: {
        /**
         * CartItem
         * @description Represents a single item in the cart with pricing details.
         */
        CartItem: {
            /** Item */
            item: string;
            /** Quantity */
            quantity: number;
            /** Original Price */
            original_price: number;
            /** Discounted Price */
            discounted_price: number;
        };
        /**
         * CartResponse
         * @description Generic response model for actions like adding or clearing the cart.
         */
        CartResponse: {
            /** Message */
            message: string;
        };
        /** CatalogItem */
        CatalogItem: {
            /** Code */
            code: string;
            /** Name */
            name: string;
            /** Price */
            price: number;
        };
        /** HTTPValidationError */
        HTTPValidationError: {
            /** Detail */
            detail?: components["schemas"]["ValidationError"][];
        };
        /**
         * ListCartResponse
         * @description Represents the response for the `/cart/list` endpoint.
         */
        ListCartResponse: {
            /** Items */
            items: components["schemas"]["CartItem"][];
        };
        /**
         * ListCatalogResponse
         * @description Represents the response for the `/catalog/list` endpoint.
         */
        ListCatalogResponse: {
            /** Items */
            items: components["schemas"]["CatalogItem"][];
        };
        /** ProductInput */
        ProductInput: {
            /** Code */
            code: string;
            /** Quantity */
            quantity: number;
        };
        /** ValidationError */
        ValidationError: {
            /** Location */
            loc: (string | number)[];
            /** Message */
            msg: string;
            /** Error Type */
            type: string;
        };
    };
    responses: never;
    parameters: never;
    requestBodies: never;
    headers: never;
    pathItems: never;
}
export type $defs = Record<string, never>;
export interface operations {
    add_to_cart_cart_add_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["ProductInput"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["CartResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    list_cart_cart_list_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ListCartResponse"];
                };
            };
        };
    };
    clear_cart_cart_clear_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["CartResponse"];
                };
            };
        };
    };
    list_catalog_catalog_list_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ListCatalogResponse"];
                };
            };
        };
    };
}
